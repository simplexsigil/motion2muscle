...
from models.m2m_mamba import MixerModel
...
def print_trainable_parameters(model, logger):
    ...
    for name, param in model.named_parameters():
        ...
        if param.requires_grad:
            ...
            logger.info(f"Layer: {name} | Size: {param.size()} | Parameters: {format_number(param.numel())}")
    logger.info(
        f"trainable params: {format_number(trainable_params)} || all params: {format_number(all_param)} || trainable%: {100 * trainable_params / all_param:.2f}"
    )
...
def initialize_model_and_optimizer(args, logger):
    input_width = 263
    output_width = 402
    net = MixerModel(d_input=input_width, d_output=output_width)

    print_trainable_parameters(net, logger)
